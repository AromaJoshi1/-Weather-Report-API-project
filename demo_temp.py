...
def data_get():
    city = city_name.get()
    data = requests.get(
        "https://api.openweathermap.org/data/2.5/weather?q="+city+"&appid=893d4e3c6cf548eee6c130f6589fe649").json()
    w_label1.config(text=data["weather"][0]["main"])
    wb_label1.config(text=data["weather"][0]["description"])
    temp_label1.config(text=str(int(data["main"]["temp"]-273.15)))
    per_label1.config(text=data["main"]["pressure"]-273.15)


win = Tk()
win.title("Aroma Weather App")
win.config(bg="skyblue")
win.geometry("500x570")

name_label = Label(win, text="Aroma Weather App",
...
com = ttk.Combobox(win, text="Aroma Weather App",
                   values=list_name,
